...
public class WebDavTreeBean extends AbstractSessionBean implements Serializable, ActionListener {
	...
	/*
	 * public void setTree(WebTree theTree) { if (tree == null){
	 ...
	 * theTree.setPlusIcon("../images/owtplusxp.gif"); initTree(theTree); TreeNode
	 * rootNode = (TreeNode) theTree.getChildren().get(0);
	 * 
	 *  // rootNode.hide();
	 *  } else { //refresh the tree, starting from root ((SlideTreeNode)
	 * tree.getChildren().get(0)).refresh(); } tree = theTree; }
	 */

	/*
	 * private void initTree(WebTree tree) { FacesContext fContext =
	 * javax.faces.context.FacesContext.getCurrentInstance(); try {
	 * tree.getChildren().add(getSubTree(getRoot(), fContext)); //
	 * test(getRoot()); } catch (Exception ex){ tree.getChildren().add(new
	...
	documentmanagementprototype2.WebDavTreeBean.SlideTreeNode getSubTree(WebdavResource resource, FacesContext fContext) throws IOException, HttpException {
		...
		// node.setActionListener(getApplication().createMethodBinding("#{WebDavTree.processAction}",
	...
	public void processAction(javax.faces.event.ActionEvent actionEvent) throws javax.faces.event.AbortProcessingException {
		/*
		 * documentmanagementprototype2.WebDavTreeBean.SlideTreeNode node =
		 * (SlideTreeNode) actionEvent.getComponent(); WebdavResource resource =
		 * node.getWebdavResource(); try{ _selectedCollectionUri =
		 * resource.getHttpURL().getURI(); java.lang.System.out.print("..."); }catch
		 * (org.apache.commons.httpclient.URIException ex){
	...
	/*
	 * private TreeNode getNode(TreeNode tree, String id){ TreeNode foundNode;
	 * java.util.Iterator children = tree.getChildren().iterator(); while
	 * (children.hasNext()){ TreeNode child = (TreeNode) children.next(); if
	 * (child.getId().equals(id)){ return child; } else { return getNode(child,
	 * id); } } }
	 */
	...
	class SlideTreeNode {// extends TreeNode {
		 ...
		 * if(_visible){ super.encodeBegin(fContext); }else{
