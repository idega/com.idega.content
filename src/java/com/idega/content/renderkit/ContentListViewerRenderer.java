/*
 * $Id: ContentListViewerRenderer.java,v 1.10.2.1 2007/05/21 07:57:42 laddi Exp $ Created on 27.1.2005
 * 
...
/**
 * 
 * Last modified: $Date: 2007/05/21 07:57:42 $ by $Author: laddi $
 * 
			...
			for (int j = 0, size = component.getChildCount(); j < size; j++) {
				...
				if (child instanceof UIColumn && ((UIColumn) child).isRendered()) {
					...
					for (int k = 0; k < child.getChildCount(); k++) { // because there is only one child allways
						...
						buf.append(" ").append(savedArticleItemStyleClass).append("_").append(i + 1);
...
// renderFacet(facesContext, writer, (UIData) uiComponent, false);
	...
	protected void renderViewerListHeaderRow(FacesContext facesContext, ResponseWriter writer, UIData uiData, UIComponent headerFacet, String headerStyleClass) throws IOException {
		renderViewerListHeaderOrFooterRow(facesContext, writer, uiData, headerFacet, headerStyleClass, DIV_ELEM);
	}

	protected void renderViewerListFooterRow(FacesContext facesContext, ResponseWriter writer, UIData uiData, UIComponent footerFacet, String footerStyleClass) throws IOException {
	...
	private void renderViewerListHeaderOrFooterRow(FacesContext facesContext, ResponseWriter writer, UIData uiData, UIComponent facet, String styleClass, String colElementName) throws IOException {
// HtmlRendererUtils.writePrettyLineSeparator(facesContext);
