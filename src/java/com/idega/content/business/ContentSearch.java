/*
 * $Id: ContentSearch.java,v 1.20.2.17 2007/05/29 08:13:19 laddi Exp $ Created on Jan 17, 2005
 * 
...
/**
 * 
 * Last modified: $Date: 2007/05/29 08:13:19 $ by $Author: laddi $ This class implements the Searchplugin interface and can therefore be used in a
 * Search block (com.idega.core.search)<br>
 * for searching contents and properties (metadata) of the files in the iwfile system. To use it simply register this class as a iw.searchable
 * component in a bundle.
 * 
 * TODO Load the dasl searches from files! (only once?)
 * 
 * @author <a href="mailto:eiki@idega.com">Eirikur S. Hrafnsson</a>
 * @version $Revision: 1.20.2.17 $
 */
public class ContentSearch extends Object implements SearchPlugin {
	...
	protected String getAdvancedContentSearch(SearchQuery searchQuery) throws SearchException {
		...
		if (queryString != null && !"".equals(queryString)) {
			...
			for (int i = 0; i < tokens.length; i++) {
				...
				orExpression = s.or(orExpression, s.or(s.compare(CompareOperator.LIKE, DISPLAYNAME, wildCardSearchWord), s.or(s.compare(CompareOperator.LIKE, CREATOR_DISPLAY_NAME, searchWord), s.compare(CompareOperator.LIKE, COMMENT, wildCardSearchWord))));
				if (expression != null) {
	...
	protected String getPropertySearch(SearchQuery searchQuery) throws SearchException {
		...
		for (int i = 0; i < tokens.length; i++) {
			String searchWord = tokens[i];
			String wildCardSearchWord = getWildCardSearchWord(searchWord);
			SearchExpression orExpression = s.or(s.compare(CompareOperator.LIKE, DISPLAYNAME, wildCardSearchWord), s.or(s.compare(CompareOperator.LIKE, CREATOR_DISPLAY_NAME, searchWord), s.compare(CompareOperator.LIKE, COMMENT, wildCardSearchWord)));
			if (expression != null) {
				expression = s.and(orExpression, expression);
			}
			else {
	...
	protected String getCombinedContentAndPropertySearch(SearchQuery searchQuery) throws SearchException {
		...
		searchXML.append("<D:searchrequest xmlns:D='DAV:' xmlns:S='http://jakarta.apache.org/slide/'><D:basicsearch><D:select><D:prop><D:getcontentlength/><D:creationdate/><D:displayname/><D:getlastmodified/></D:prop></D:select>").append("<D:from>").append("<D:scope><D:href>").append(getScopeURI()).append("</D:href><D:depth>infinity</D:depth></D:scope>").append("</D:from>").append("<D:where>");
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			searchXML.append("<D:contains>").append(searchWord).append("</D:contains>").append("<S:property-contains><D:prop><D:displayname/></D:prop><D:literal>").append(searchWord).append("</D:literal></S:property-contains>").append("<S:property-contains><D:prop><D:creator-displayname/></D:prop><D:literal>").append(searchWord).append("</D:literal></S:property-contains>").append("<S:property-contains><D:prop><D:comment/></D:prop><D:literal>").append(searchWord).append("</D:literal></S:property-contains>");
		}

		searchXML.append("</D:or>");
		if (isSetToIgnoreFolders()) {
			searchXML.append("<D:not-is-collection/>").append("</D:and>");
		}

		searchXML.append("</D:where>").append("</D:basicsearch>").append("</D:searchrequest>");
	...
	protected String addOrderingAndLimitingToDASLSearchXML(String searchXML) {
		...
		/*
		 * String orderAscendingOrDescending = ContentSearch.ORDER_ASCENDING; if(isSetToUseDescendingOrder()){ orderAscendingOrDescending =
		 * ContentSearch.ORDER_DESCENDING; }
		 * 
		 * StringBuffer extraXML = new StringBuffer(); extraXML.append("<D:orderby><D:order><D:prop><D:").append(getPropertyToOrderBy()).append("/></D:prop>")
		 * .append("<D:").append(orderAscendingOrDescending).append("/></D:order></D:orderby>");
		 * 
		 * if(getNumberOfResultItemsToReturn()>0){ extraXML.append("<D:limit><D:nresults>").append(getNumberOfResultItemsToReturn()).append("</D:nresults></D:limit>"); }
		 * 
		 * int index = searchXML.indexOf(DASL_WHERE_XML_SNIPPET); searchXML = searchXML.substring(0, index + DASL_WHERE_XML_SNIPPET.length()) +
		 * extraXML.toString()+ searchXML.substring(index + DASL_WHERE_XML_SNIPPET.length()); return searchXML;
		 */
	 ...
	 * 
	 * @param numberOfResultItems
	 *          the numberOfResultItems to set
	 */
	 ...
	 * 
	 * @param hideFileExtension
	 *          the hideFileExtension to set
	 */
