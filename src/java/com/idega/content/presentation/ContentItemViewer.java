/*
 * $Id: ContentItemViewer.java,v 1.35 2008/06/06 16:22:48 valdas Exp $ Created
 * on 26.1.2005
 *
...
/**
 *
 * Last modified: $Date: 2008/06/06 16:22:48 $ by $Author: valdas $
 *
...
public class ContentItemViewer extends WFContainer {
		...
		if (binding != null) {
			...
			if ("value".equals(bindingAttribute)) {
				if (uiValueBound instanceof UIOutput) {
					...
					invoked = true;
				} else if (uiValueBound instanceof Text) {
					((Text) uiValueBound).setText((String) getValue(fieldName));
					invoked = true;
				}
			}
	...
	private UIComponent getValueBindingComponent(UIComponent output) {
		if(output instanceof HtmlOutputLink){
			...
			return output.getChildren().get(0);
		}
		else{
			return output;
		}
				...
				if (component instanceof ContentItemFieldViewer) {
					...
					if ("value".equals(bindingAttribute)) {
						if (value instanceof UIOutput) {
							...
							((Text) value).setText((String) getValue(fieldName));
							invoked = true;
						}
					} else if ("url".equals(bindingAttribute)) {
	...
	public Object saveState(FacesContext ctx) {
		...
		values[11] = Boolean.valueOf(this.identifierAutoGenerated);
		return values;
	}
	...
	public void restoreState(FacesContext ctx, Object state) {
		...
		this.identifierAutoGenerated = values[11] == null ? false : Boolean.valueOf(values[11].toString());
	}
